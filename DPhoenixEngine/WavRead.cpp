...
//-----------------------------------------------------------------------------
HRESULT WaveSoundRead::Open(CHAR* strFilename)
{
	SAFE_DELETE(mWfx);

	HRESULT  hr;

	if (FAILED(hr = WaveOpenFile(strFilename, &mHmmioIn, &mWfx, &mCkInRiff)))
		return hr;
...
//-----------------------------------------------------------------------------
HRESULT WaveSoundRead::Reset()
{
	return WaveStartDataRead(&mHmmioIn, &mCkIn, &mCkInRiff);
}
...
//-----------------------------------------------------------------------------
HRESULT WaveSoundRead::Read(UINT nSizeToRead, BYTE* pbData, UINT* pnSizeRead)
{
	return WaveReadFile(mHmmioIn, nSizeToRead, pbData, &mCkIn, pnSizeRead);
}
...
//-----------------------------------------------------------------------------
HRESULT WaveSoundRead::Close()
{
	mmioClose(mHmmioIn, 0);
	return S_OK;
}
