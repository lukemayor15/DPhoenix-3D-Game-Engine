...
class LITSAI : public DPhoenix::Engine
{
private:
...
#pragma region BTLTMembers
	...
	DPhoenix::Sprite* mWinScreen;
	DPhoenix::Sprite* mLooseScreen;
...
#pragma region LITSMapMembers
	...
	std::vector<std::string> mStrongWindColorMaps;
	std::vector<std::string> mStrongWindNormalMaps;
	std::vector<std::string> mHealColorMaps;
	std::vector<std::string> mHealNormalMaps;
	std::vector<std::string> mBaseColorMaps;
	std::vector<std::string> mBaseNormalMaps;
...
#pragma region LITSActions
	...
	bool mA1Hover, mA2Hover, mA3Hover, mA4Hover,
		mA5Hover, mA6Hover, mA7Hover, mA8Hover;
	...
	DPhoenix::LITSMap* map;

	//for click contorl 
	bool mIsMenuDone = false; // so it allows user to change level, remove last when last player been choosen
	...
	BeaconStages mBeaconStages;
	DPhoenix::ParticleEmitter* mParticleEmitter;
	//weapones
...
#pragma region LITSAI
	//falg to se on game over to idicate win
	bool mIsPlayerWinner;
#pragma endregion LITSAI
#pragma endregion LITSMembers

public:
	LITSAI(HINSTANCE hInstance);
	~LITSAI();
	...
	void LITSAI::HandleEvents(DPhoenix::IEvent* e);
	...
	void LITSAI::RenderBasicTexPrimitive(DPhoenix::PrimitiveInstance * model,
		ID3DX11EffectTechnique * tech);

	//new method / specifically handles textured primitives with directional lights
	//and fog
	void LITSAI::RenderLitTexPrimitive(DPhoenix::PrimitiveInstance * model,
		ID3DX11EffectTechnique * tech);

	//new method / now with normal mapping!
	void LITSAI::RenderLitTexNormPrimitive(DPhoenix::PrimitiveInstance * model,
		ID3DX11EffectTechnique * tech);
	...
	void LITSAI::RenderBillboardSprite(DPhoenix::BillboardSprite * model,
		ID3DX11EffectTechnique * tech);

	//new method / render static mesh with subsets
	void LITSAI::RenderBasicModel(DPhoenix::BasicModelInstance * model,
		ID3DX11EffectTechnique * tech);

	//new method / render static mesh with subsets
	void LITSAI::RenderBasicModelNormal(DPhoenix::BasicModelInstance * model,
		ID3DX11EffectTechnique * tech);

	//new method / render animated lit mesh with subsets
	void LITSAI::RenderSkinnedModel(DPhoenix::SkinnedModelInstance * model,
		ID3DX11EffectTechnique * tech);

	//new method / render animated lit normal mapped mesh with subsets
	void LITSAI::RenderSkinnedModelNormal(DPhoenix::SkinnedModelInstance * model,
		ID3DX11EffectTechnique * tech);


#pragma region LITSMethods

	bool LITSAI::CheckPointInRect(int pointX, int pointY, RECT rect);
	void LITSAI::CheckMouseHoverMenu();
	void LITSAI::CheckMouseClickMenu();

	void LITSAI::CheckMouseHoverTurns();
	void LITSAI::CheckMouseClickTurns();

	//for movment
	void LITSAI::CheckMouseClickRoute();
	void LITSAI::CheckSelectableRoute();
	void LITSAI::ResetPath();

	//for actions
	void LITSAI::CheckMouseHoverActions();
	void LITSAI::CheckMouseClickActions();

	void LITSAI::ResetTargeting();
	void LITSAI::CheckMouseClickTarget();

	XMFLOAT2 LITSAI::Convert3Dto2DPoint(XMFLOAT3 pos3d);

	//for restarting all game context
	void ReInit();
...
LITSAI::LITSAI(HINSTANCE hInstance)
	: Engine(hInstance)
{
	mMainWndCaption = "Ledgends In the Sky";
...
bool LITSAI::Init(bool fullScreen)
{
...
#pragma region LITSInit
	...
	mCrusierMenu->mModelInstance->Model->Mat[0] = *materialStandard;
	mBrusierMenu->mModelInstance->Model->Mat[0] = *materialStandard;;
	mCorvetteMenu->mModelInstance->Model->Mat[0] = *materialStandard;;
	mFlagShipMenu->mModelInstance->Model->Mat[0] = *materialStandard;
...
#pragma region LITSMapInit
	mSkybox = new DPhoenix::Sky(md3dDevice, "textures\\Skybox\\Sky.dds", 5000.0f);

	mFloorColorMaps.insert(mFloorColorMaps.end(),
		{ "Textures\\Floor\\Floor_cm.jpg" });

	mFloorNormalMaps.insert(mFloorNormalMaps.end(),
		{ "Textures\\Floor\\Floor_nm.jpg" });

	mHealColorMaps.insert(mHealColorMaps.end(),
		{ "Textures\\Floor\\Heal_cm.jpg" });

	mHealNormalMaps.insert(mHealNormalMaps.end(),
		{ "Textures\\Floor\\Heal_nm.jpg" });
	mBaseColorMaps.insert(mBaseColorMaps.end(),
		{ "Textures\\Floor\\Base_cm.jpg" });

	mBaseNormalMaps.insert(mBaseNormalMaps.end(),
		{ "Textures\\Floor\\Base_nm.jpg" });
	mStrongWindColorMaps.insert(mStrongWindColorMaps.end(),
		{ "Textures\\Floor\\StrongWind_cm.jpg" });

	mStrongWindNormalMaps.insert(mStrongWindNormalMaps.end(),
		{ "Textures\\Floor\\StrongWind_nm.jpg" });

	mWallColorMaps.insert(mWallColorMaps.end(),
		{ "Textures\\Wall\\Wall_cm.jpg" });
	mWallNormalMaps.insert(mWallNormalMaps.end(),
		{ "Textures\\Wall\\Wall_nm.jpg" });
	mCoverColorMaps.insert(mCoverColorMaps.end(),
		{ "Textures\\Cover\\CoverTex.jpg" });
	mCoverNormalMaps.insert(mCoverNormalMaps.end(),
		{ "Textures\\Cover\\CoverTex_nm.jpg" });
	...
	mMap = new DPhoenix::LITSMap("Data\\Levels\\Level2.csv", &mTexMgr, md3dDevice,
		mEnemySpawnVec, mPlayerSpawnVec, mBeaconSpawnVec,
		mBox, mFloorColorMaps, mFloorNormalMaps,mStrongWindColorMaps, mStrongWindNormalMaps,
		mHealColorMaps, mHealNormalMaps, mBaseColorMaps, mBaseNormalMaps,mWallColorMaps, mWallNormalMaps,
		mCoverColorMaps, mCoverNormalMaps, 30, 30, 20.0f);
...
#pragma region LITSActionsInit
	//select flags init
	mGotActions = false;
	mA1Hover = mA2Hover = mA3Hover = mA4Hover = mA5Hover = mA6Hover = mA7Hover, mA8Hover = false;
	...
	//create the ple and sphere for the beacons
	...
	{
		mBeacons.push_back(new DPhoenix::Beacon(mBeaconSpawnVec[i], &mTexMgr,
			md3dDevice, mBeaconSphere, mBeaconPole));
		AddEntity(mBeacons.back()->mPoleMeshInstance);
	}

	mBeaconStages = GOTO_TARGET_BEC;

	//need to add map enties into engine
...
#pragma endregion LITSInit
	...
	mAudioMgr.GetSound("MainTheme")->Play(true);
	return true;
}
	...
	for (int i = 0; i < mSelectionBoxes.size(); i++)
	{
		if (mSelectionBoxes[i]->mCollided)
		{
			...
			if (mMap->mTiles[col][row]->mSpecialFlag == DPhoenix::NO_SP_FLAG &&
				...
				mMap->mTiles[col][row]->mMapBlockType == DPhoenix::STRONGWIND_MAPBLOCK ||
				mMap->mTiles[col][row]->mSpecialFlag == DPhoenix::NO_SP_FLAG &&
				mMap->mTiles[col][row]->mMapBlockType == DPhoenix::HEAL_MAPBLOCK||
				mMap->mTiles[col][row]->mSpecialFlag == DPhoenix::NO_SP_FLAG &&
				mMap->mTiles[col][row]->mMapBlockType == DPhoenix::BASE_MAPBLOCK)
...
void LITSAI::CheckMouseHoverActions()
{
	...
	if (mCurrentTeam == DPhoenix::PLAYER_TEAM &&
		...
		mPlayerTeam->mTeamMembers[mPlayerTeam->mCurrentMember]->mMoveState == DPhoenix::CH_PICKAC_MOVESTATE)
	{
		if (CheckPointInRect(mouseX, mouseY, LITSConst::actionSelect8Section))
		{
			mA8Hover = true;
...
void LITSAI::CheckMouseClickActions()
{
		...
		mPlayerTeam->mTeamMembers[mPlayerTeam->mCurrentMember]->mMoveState == DPhoenix::CH_PICKRT_MOVESTATE)
	{
		if (mA1Hover) 
		{
			mPlayerTeam->CheckSelectedAction(0, map);
			bool mSecClick = true;

		}
		if (mA2Hover) mPlayerTeam->CheckSelectedAction(1, map);
		if (mA3Hover) mPlayerTeam->CheckSelectedAction(2, map);
		if (mA4Hover) mPlayerTeam->CheckSelectedAction(3, map);
		if (mA5Hover) mPlayerTeam->CheckSelectedAction(4, map);
		if (mA6Hover) mPlayerTeam->CheckSelectedAction(5, map);
		if (mA7Hover) mPlayerTeam->CheckSelectedAction(6, map);
		if (mA8Hover)
		{
...
void LITSAI::ResetTargeting()
{
			...
			for (int row = minRow; row <= maxRow; row++)
			{
				...
				if (!(currentCol == col && currentRow == row))
				{
					...
					if ((mMap->mTiles[col][row]->mSpecialFlag == DPhoenix::ENEMY_SP_FLAG &&
						mPlayerTeam->mTeamMembers[mPlayerTeam->mCurrentMember]->mSelectedAction == DPhoenix::HIT_ACTION) ||
						(mMap->mTiles[col][row]->mMapBlockType == DPhoenix::BEACON_MAPBLOCK &&
							mPlayerTeam->mTeamMembers[mPlayerTeam->mCurrentMember]->mSelectedAction == DPhoenix::LIGHT_BEACON_ACTION))
					{
	...
	case DPhoenix::HEAVYCANNON_ACTION:
	case DPhoenix::SUBCANNON_ACTION:
	case DPhoenix::QUICK_FIRIING_ARTILLERY_ACTION:




		//set checking for only adjacent squares of vaild tiles
		minRow = max(currentRow - 3, 0);
		maxRow = min(currentRow + 3, mMap->mWidth - 1);
		minCol = max(currentCol - 3, 0);
		maxCol = min(currentCol + 3, mMap->mLength - 1);
	...
	case DPhoenix::TORPEDO_ACTION:
	case DPhoenix::BARRAGE_ACTION:
	case DPhoenix::ALL_WEAPONS_FIRE_ACTION:


		//set checking for only adjacent squares of vaild tiles
		minRow = max(currentRow - 7, 0);
		maxRow = min(currentRow + 7, mMap->mWidth - 1);
		minCol = max(currentCol - 7, 0);
		maxCol = min(currentCol + 7, mMap->mLength - 1);
		...
		break;
	//case DPhoenix::SNIPE_ACTION:
	//case DPhoenix::COMMAND_ACTION:
	//case DPhoenix::BRACE_ACTION:
	//case DPhoenix::EMERGENCY_REPAIR_ACTION:
	//	//set checking for only adjacent squares of vaild tiles
	//	minRow = max(currentRow - 0, 0);
	//	maxRow = min(currentRow + 0, mMap->mWidth -1 );
	//	minCol = max(currentCol - 0, 0);
	//	maxCol = min(currentCol  + 0, mMap->mLength - 1 );
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (validMove)
			{
				...
				if (targetMapBlock->mMapBlockType == DPhoenix::FLOOR_MAPBLOCK &&
					targetMapBlock->mSpecialFlag == DPhoenix::NO_SP_FLAG ||
					targetMapBlock->mMapBlockType == DPhoenix::STRONGWIND_MAPBLOCK &&
					targetMapBlock->mSpecialFlag == DPhoenix::NO_SP_FLAG ||
					targetMapBlock->mSpecialFlag == DPhoenix::NO_SP_FLAG &&
					targetMapBlock->mMapBlockType == DPhoenix::BASE_MAPBLOCK ||
					targetMapBlock->mSpecialFlag == DPhoenix::NO_SP_FLAG &&
					targetMapBlock->mMapBlockType == DPhoenix::HEAL_MAPBLOCK)
				{
...
void LITSAI::UpdateScene(float dt)
{
	...
	case GAME_MENU_STATE:
		...
		case ENEMY_4_PICKED_MENUSTATE:
			if (mWaitTimer.TotalTime() > 2.0f)
			{
				...
				//stop audio of main theme
				mAudioMgr.GetSound("MainTheme")->Stop();
				mAudioMgr.GetSound("MainTheme")->SetPosition(0);
				...
				for (int i = 0; i < mPlayerTeam->mTeamMembers.size(); i++)
				{
					...
					mPlayerTeam->mTeamMembers[i]->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::IDLE_ANIMATION);
				}

				for (int i = 0; i < mEnemyTeam->mTeamMembers.size(); i++)
				{
					mEnemyTeam->mTeamMembers[i]->mModelInstance->mRotYOffset = 0.0f;
					mEnemyTeam->mTeamMembers[i]->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::IDLE_ANIMATION);
				}
				...
				for (int i = 0; i < mPlayerTeam->mTeamMembers.size(); i++)
				{
					...
					mPlayerTeam->mTeamMembers[i]->mModelInstance->mHalfSizes = XMFLOAT3(5.0f, 20.0f, 5.0f);
				...
				for (int i = 0; i < mEnemyTeam->mTeamMembers.size(); i++)
				{
					...
					mEnemyTeam->mTeamMembers[i]->mModelInstance->mHalfSizes = XMFLOAT3(5.0f, 20.0f, 5.0f);
	...
	case GAME_PLAY_STATE:
		mDirLights[0]->Direction = XMFLOAT3(0.0f, -0.77f, -0.77f);
...
#pragma region CheckTeam
		...
		if (mCurrentTeam == DPhoenix::PLAYER_TEAM)
		{
			...
			mAudioMgr.GetSound("PlayerTurn")->Play();
			mAudioMgr.GetSound("EnemyTurn")->Stop();
			mAudioMgr.GetSound("EnemyTurn")->SetPosition(0);
		}
		else if (mCurrentTeam == DPhoenix::ENEMY_TEAM)
		{
			activeTeam = mEnemyTeam;
			mAudioMgr.GetSound("EnemyTurn")->Play();
			mAudioMgr.GetSound("PlayerTurn")->Stop();
			mAudioMgr.GetSound("PlayerTurn")->SetPosition(0);
		}
			...
			&& mActionStage == AC_PREPARE)
		{
				...
				mCamera->mPosition.z == mCamera->mGoalPosition.z)
			{
				...
				case DPhoenix::HIT_ACTION:
					...
					break;
				case DPhoenix::LIGHT_BEACON_ACTION:
					mBeaconStartPos = currentCharacter->mModelInstance->mPosition;
					mBeaconStages = GOTO_TARGET_BEC;
					currentTeam->mTPPool -= 1;
					break;
				case DPhoenix::CANNON_ACTION:
					//construct sending current character, the direction to fire, the srtart pos
					//the type of action selected, the current team type, pointers TO TEAMS AND SINGLETOUNS
					mWeaponLines.push_back(new DPhoenix::WeaponLine(currentCharacter,
						currentCharacter->mModelInstance->mForwardVector,
						currentCharacter->mModelInstance->mPosition,
						currentCharacter->mSelectedAction,
						currentTeam->mTeamType, mPlayerTeam, mEnemyTeam, &mTexMgr, md3dDevice));

					AddEntity(mWeaponLines.back()->mWeaponLine);

					//otherwise we keep moving towards the oppent - hence set walking aniation
					if (currentCharacter->mModelInstance->mInstanceAnimator->GetAnimationIndex() != DPhoenix::WEAPON_ANIMATION)
					{
						currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::WEAPON_ANIMATION);
						currentCharacter->mModelInstance->SetTimePos(0);
						currentCharacter->mModelInstance->SetLoopClips(false);

						//quirk of aniation in modle it face the wrong way
						if (currentCharacter->mClass == DPhoenix::FLAG_SHIP_CLASS)
							currentCharacter->mModelInstance->mRotYOffset = 90.0f;
					}
				...
				case DPhoenix::ALL_WEAPONS_FIRE_ACTION:
					...
					//sphere needed as base primtive for the weapon prjectile and to send to constructor
					...
					//construct sending current char, the direction to firem the staert pos
					//the type of action s3lct the currtn team type pointer to team and single and hte sphere
					mWeaponMeshes.push_back(new DPhoenix::WeaponMesh(currentCharacter,
						currentCharacter->mModelInstance->mForwardVector,
						currentCharacter->mModelInstance->mPosition,
						currentCharacter->mSelectedAction,
					...
					DPhoenix::CharacterAnimations requiredAnimation;
					if (mWeaponMeshes.back()->mWeaponType == DPhoenix::MAGIC_WT)
						requiredAnimation = DPhoenix::MAGIC_ANIMATION;
					else
						requiredAnimation = DPhoenix::WEAPON_ANIMATION;

					//ohter wise we keepmoving toward the oppent - hence set walking aniamtiuin
					if (currentCharacter->mModelInstance->mInstanceAnimator->GetAnimationIndex() != requiredAnimation)
					{
						currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(requiredAnimation);
						currentCharacter->mModelInstance->SetTimePos(0);
						currentCharacter->mModelInstance->SetLoopClips(false);

						//quirk of aniamiton in model is it face the wrong wat
						if (currentCharacter->mClass == DPhoenix::BRUSIER_SHIP_CLASS)
							currentCharacter->mModelInstance->mRotYOffset = 90.0f;
					}
					break;
...
#pragma region ActionFireStage
			...
			&& mActionStage == AC_FIRE)
		{
			...
			if (!mAudioMgr.GetSound("Fire")->IsPlaying())
			{
				mAudioMgr.GetSound("Fire")->Play(true);
			...
			if (weaponDoneFlag)
			{
				mActionStage = AC_POSTFIRE;
				mActionTimer.Reset();
				mAudioMgr.GetSound("Fire")->Stop();
				mAudioMgr.GetSound("Fire")->SetPosition(0);

			}
			//beacon actions
			if (currentCharacter->mSelectedAction == DPhoenix::LIGHT_BEACON_ACTION)
			{
				bool beaconCollision = false;

				switch (mBeaconStages)
				{
				case GOTO_TARGET_BEC:
					//initail loop thorugh
					for (int i = 0; i < mBeacons.size(); i++)
					{
						for (int j = 0; j < mBeacons[i]->mPoleMeshInstance->mAABBCollidedEntities.size(); j++)
						{
							//for each beacon of the collided entites for each beacon, check is collided with ucrrent char
							if (mBeacons[i]->mPoleMeshInstance->mAABBCollidedEntities[j]->mId ==
								currentCharacter->mModelInstance->mId)
							{
								//if so, light the beacon with given team falg
								if (mCurrentTeam == DPhoenix::PLAYER_TEAM)
					...
					//if we have collided then we update hte stage and aniamtion
					if (beaconCollision)
					{
						mBeaconStages = LIGHT_TARGET_BEC;
						currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::MAGIC_ANIMATION);
						currentCharacter->mModelInstance->SetTimePos(0);
						currentCharacter->mModelInstance->SetLoopClips(true);
						mActionTimer.Reset();
					}
					...
					{
						//otherwise we keep moving towards the beacon - hence set walking animation
						if (currentCharacter->mModelInstance->mInstanceAnimator->GetAnimationIndex() != DPhoenix::WALK_ANIMATION)
						{
							currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::WALK_ANIMATION);
							currentCharacter->mModelInstance->SetTimePos(0);
				...
				case LIGHT_TARGET_BEC:
					...
					//after a certain time elpased update stage and aniation
					if (mActionTimer.TotalTime() > 2.5f)
					{
						mBeaconStages = RETURN_BEC;
						currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::WALK_ANIMATION);
						currentCharacter->mModelInstance->SetTimePos(0);
						currentCharacter->mModelInstance->SetLoopClips(true);
						mActionTimer.Reset();
				...
				case RETURN_BEC:
					...
					//the goal is back to the position we started the light pos
					//so get the distance between our current pos and that
			...
			for (int i = 0; i < mWeaponMeshes.size(); i++)
			{
				...
				if (mWeaponMeshes[i]->mWeaponStage == DPhoenix::FIRING_MESH_WMS)
				{
					currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::IDLE_ANIMATION);
					currentCharacter->mModelInstance->SetTimePos(0);
					currentCharacter->mModelInstance->SetLoopClips(true);
...
#pragma region ActionPostFireStage
			...
			&& mActionStage == AC_POSTFIRE)
		{
			...
			//update lines still for billboar animationd
			for (int i = 0; i < mWeaponLines.size(); i++)
			{
				mWeaponLines[i]->Update(dt);
			...
			if (mActionTimer.TotalTime() > 0.5f)
			{
				...
				currentTeam->EndCurrentTurn(map);
				...
				//unset alive status for all line entities then clear the vector
				for (int i = 0; i < mWeaponLines.size(); i++)
				{
				...
				mWeaponLines.clear();
				//unset alive status for all wapeon meshes enetue then clear the vector
				for (int i = 0; i < mWeaponMeshes.size(); i++)
				{
				...
				mWeaponMeshes.clear();
				if (currentCharacter->mModelInstance->mInstanceAnimator->GetAnimationIndex() != DPhoenix::IDLE_ANIMATION)
				{
					currentCharacter->mModelInstance->mInstanceAnimator->SetAnimation(DPhoenix::IDLE_ANIMATION);
					currentCharacter->mModelInstance->SetTimePos(0);
					currentCharacter->mModelInstance->SetLoopClips(true);
...
#pragma endregion EnemyAIMovment
...
#pragma endregion ActionsUpdating
		...
		if (currentCharacter->mTurnState == DPhoenix::CH_COMP_TURNSTATE)
		{

			if (!mPlayerTeam->CheckTeamState())
			{
				mGameState = GAME_OVER_STATE;
				mIsPlayerWinner = false;
			} 

			if (!mEnemyTeam->CheckTeamState())
			{
				mGameState = GAME_OVER_STATE;
				mIsPlayerWinner = true;
			}
...
void LITSAI::BuildGeometryBuffers()
{
...
#pragma region Beacons
		//toogle fog off for beacons so more bisbale at distance
		mFogEnabled = false;
...
#pragma region RenderBillboards
	...
	//all billboard sprites for the weapon lines
	for (int i = 0; i < mWeaponLines.size(); i++)
	{
		for (int j = 0; j < mWeaponLines[i]->mBillboards.size(); j++)
		{
			...
			/*for each(DPhoenix::BillboardSprite* p in mWeaponLines[i]->mParticleEmitter->mParticles)
			{
				RenderBillboardSprite(p, activeLitBillboardTech);
			}*/
	...
	//all billboard sprites for the weapon meshes
	for (int i = 0; i < mWeaponMeshes.size(); i++)
	{
	...
	/*for each (DPhoenix::BillboardSprite* p in mParticleEmitter->mParticles)
	{
		RenderBillboardSprite(p, activeLitBillboardTech);
	}*/
...
#pragma region DmgText
		...
		for (int i = 0; i < mWeaponLines.size(); i++)
		{
			...
			if (mWeaponLines[i]->isHit)
			{
				...
				//if greater than -1 it is a hit we need to dsiplayed the bvalue of
		...
		for (int i = 0; i < mWeaponMeshes.size(); i++)
		{
			//loop through all hit
			for (int j = 0; j < mWeaponMeshes[i]->mDmgPosition.size(); j++)
			{
				//clear the stringstream
				dmgText.clear(); dmgText.str("");
				...
				//covnert the pos in 3D 2D
				XMFLOAT2 dmgPos2d = Convert3Dto2DPoint(mWeaponMeshes[i]->mDmgPosition[j]);
				//and now display in red
				mUIFont30pt->DrawFont(m2D1RT, dmgText.str(), dmgPos2d.x, dmgPos2d.y,
					1.0f, 1.0f, D2D1::ColorF(0xCC0000, 1.0f));
			}
...
#pragma region BeaconText
		rowAdjust = 0.0f;
		for (int i = 0; i < mBeacons.size(); i++, rowAdjust += LITSConst::actionsBeaconsYAdjust)
		{
			...
			mUIFont30pt->DrawFont(m2D1RT, beaconsText.str(),
				LITSConst::actionsBeaconsLeft, LITSConst::actionsBeaconsTop + rowAdjust,
				1.0f, 1.0f, D2D1::ColorF(0xFFFFFF, 1.0f));
		}
#pragma endregion BeaconText
		break;
#pragma region GameOver
		case GAME_OVER_STATE:
			...
			if (mIsPlayerWinner)
			{
				mWinScreen = new DPhoenix::Sprite();
				mWinScreen->Load("Textures\\Screens\\Screen_Win.png", &mTexMgr, 1600.0f, 900.0f, md3dDevice);
				RenderSprite(mWinScreen, activeSpriteTech, true);
				mAudioMgr.GetSound("PlayerWins")->Play();


			}
			else
			{
				mLooseScreen = new DPhoenix::Sprite();
				mLooseScreen->Load("Textures\\Screens\\Screen_Loose.png", &mTexMgr, 1600.0f, 900.0f, md3dDevice);
				RenderSprite(mLooseScreen, activeSpriteTech, true);
				mAudioMgr.GetSound("EnemyWins")->Play();
			}
	...
	case DPhoenix::EVENT_KEYPRESS:
	{
		...
		if (mKeyPressTimer.TotalTime() > 0.2)
		{
			...
			case DIK_RETURN:
				...
				else if (mGameState == GAME_OVER_STATE)
				{
					ReInit();
				}
		...
		if (mMouseClickTimer.TotalTime() > 0.2)
		{
			...
			case GAME_MENU_STATE:
				if (mc->mButton == 0 && mMenuState == CHOOSE_MENUSTATE)
					CheckMouseClickMenu();
					if (mPlayerTeam->mTeamMembers.size() == 4 && mIsMenuDone == false)
						mMenuState = CONFIRM_MENUSTATE;
						mWaitTimer.Reset();
...
void LITSAI::InitAudio()
{
	mAudioMgr.CreateSound("Walk", "Audio\\FX\\Walk.wav");
	mAudioMgr.CreateSound("Hurt", "Audio\\FX\\Hurt.wav");
	mAudioMgr.CreateSound("Fire", "Audio\\FX\\Fire.wav");
	mAudioMgr.CreateSound("MainTheme", "Audio\\FX\\MainTheme.wav");
	mAudioMgr.CreateSound("PlayerTurn", "Audio\\FX\\PlayerTurn.wav");
	mAudioMgr.CreateSound("EnemyTurn", "Audio\\FX\\EnemyTurn.wav");
	mAudioMgr.CreateSound("PlayerWins", "Audio\\FX\\PlayerWins.wav");
	mAudioMgr.CreateSound("EnemyWins", "Audio\\FX\\EnemyWins.wav");
}

void LITSAI::RenderSprite(DPhoenix::Sprite * sprite, ID3DX11EffectTechnique * tech, bool isHUD)
{
...
void LITSAI::RenderColorPrimitive(DPhoenix::PrimitiveInstance * model,
	ID3DX11EffectTechnique * tech)
{
	//here we calculate the scale on X and Y based on the client width / height
	//vs. our original resolution 
...
void LITSAI::RenderSkybox(DPhoenix::Sky* skybox,
	ID3D11DeviceContext* dc)
{
	skybox->Draw(dc, mCamera);
}

void LITSAI::RenderBillboardSprite(DPhoenix::BillboardSprite * sprite,
	ID3DX11EffectTechnique * tech)
{
	//create view / projection matrix
	XMMATRIX view = mCamera->GetView();
...
void LITSAI::ReInit()
{
	for each(DPhoenix::Entity* entity in mEntities)
	{
		//KILL 
		entity->mAlive = false;
	}
	...
	mPlayerSpawnVec.clear();
	mEnemySpawnVec.clear();
	mBeaconSpawnVec.clear();
	mHappyPath.clear();
	mSelectionBoxes.clear();
	mTargets.clear();
	mTargetBoxes.clear();
	...
	//team t#reset
	mPlayerTeam->mTeamMembers.clear(); mPlayerTeam->mTurnsLeft = 4;
	mPlayerTeam->mTeamState = DPhoenix::TM_OK_TEAMSTATE;
	mPlayerTeam->mMaxTPPool = 0;
	mEnemyTeam->mTeamMembers.clear(); mEnemyTeam->mTurnsLeft = 4;
	mEnemyTeam->mTeamState = DPhoenix::TM_OK_TEAMSTATE;
	mEnemyTeam->mMaxTPPool = 0;

	//audio
	mAudioMgr.ResetAllSounds();

	//menu teams
	mBrusierMenu->SetLevel(1); mBrusierMenu->CalculateStats(1);
	mBrusierMenu->mModelInstance->mRotYOffset = -90.0f;
	mCrusierMenu->SetLevel(1); mCrusierMenu->CalculateStats(1);
	mCrusierMenu->mModelInstance->mRotYOffset = -90.0f;
	mFlagShipMenu->SetLevel(1); mFlagShipMenu->CalculateStats(1);
	mFlagShipMenu->mModelInstance->mRotYOffset = -90.0f;
	mCorvetteMenu->SetLevel(1); mCorvetteMenu->CalculateStats(1);
	mCorvetteMenu->mModelInstance->mRotYOffset = -90.0f;

	mDirLights.back()->Ambient = XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
	mDirLights.back()->Direction = XMFLOAT3(0.0f, -1.0f, 0.0f);

	mCamera->mPosition = XMFLOAT3(0.0f, 0.0f, 10.0f);
	mCamera->mGoalPosition = XMFLOAT3(0.0f, 0.0f, 10.0f);
	mCamera->mTargetPos = XMFLOAT3(10.0f, 0.0f, 10.0f);
	mCamera->mGoalTarget = mCamera->mTargetPos;
	mCamera->Update(0.0f);
	...
	//beacons - spawn them from the spwan vec in map loading
	//amd add the poles as entites for collision
	for (int i = 0; i < mBeaconSpawnVec.size(); i++)
	{
		mBeacons.push_back(
			new DPhoenix::Beacon(mBeaconSpawnVec[i], &mTexMgr,
				md3dDevice, mBeaconSphere, mBeaconPole));
		AddEntity(mBeacons.back()->mPoleMeshInstance);
	}
	...
	//need to add map enties ito engine
	for (int col = 0; col < mMap->mWidth; col++)
	{
		for (int row = 0; row < mMap->mLength; row++)
		{
	...
	mMeleeStage = GOTO_TARGET_MEL;
	mMeleeHit;
	mAudioMgr.GetSound("MainTheme")->Play();
}